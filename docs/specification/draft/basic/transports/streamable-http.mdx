...
1. Servers **MUST** validate the `Host` header on all incoming connections
...
3. When running locally, servers **SHOULD** bind only to localhost
   (127.0.0.1) rather than all network interfaces (0.0.0.0).
4. Servers **SHOULD** implement proper authentication for all connections.
