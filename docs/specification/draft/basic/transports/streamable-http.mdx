...
1. Servers **MUST** protect against DNS rebinding attacks by validating the
   `Host` header of every incoming request, on every listener, against the
   host names the server is intended to serve. Validating the `Origin` header
   instead, as required by earlier revisions of this specification, also
   satisfies this requirement.
   - If the `Host` header is not one the server serves, servers **MUST**
     reject the request with an HTTP 4xx status; 403 Forbidden is
     **RECOMMENDED**.
2. Servers that accept ambient credentials (credentials a browser attaches
   ...
   - If a server validates the `Origin` header and it is present and invalid,
     the server **MUST** respond with HTTP 403 Forbidden. The HTTP response
...
4. When running locally, servers **SHOULD** bind only to localhost
...
local or private-network MCP servers from remote websites, or use cross-site
requests to act with a user's ambient credentials.
